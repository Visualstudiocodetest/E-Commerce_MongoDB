...
\begin{frame}{Présentation de l'équipe}
  \begin{columns}
    \begin{column}{0.55\textwidth}
      \begin{itemize}
        \item \textbf{Membre 1} — Backend \& API (FastAPI)
        \item \textbf{Membre 2} — Modélisation MongoDB \& agrégations
        \item \textbf{Membre 3} — Frontend (SPA Bootstrap)
        \item \textbf{Membre 4} — Données de test \& déploiement
      \end{itemize}
      \vspace{1em}
      \footnotesize Chaque membre a contribué via Git (branches, commits, pull requests).
    \end{column}
...
\begin{frame}{Schéma de la base de données MongoDB}
  ...
  \begin{tikzpicture}[
      font=\scriptsize,
      coll/.style={draw, rounded corners, fill=mongogreen!12, minimum width=2cm, minimum height=0.7cm, align=center},
      rel/.style={-{Stealth}, gray}
    ]
    \node[coll] (users) {users};
    \node[coll, below=0.5cm of users] (addr) {addresses};
    \node[coll, below=0.5cm of addr] (carts) {shopping\_carts};
    \node[coll, right=2.4cm of users] (orders) {orders};
    \node[coll, below=0.5cm of orders] (items) {order\_items};
    \node[coll, below=0.5cm of items] (pay) {payments};
    \node[coll, right=2.4cm of orders] (products) {products};
    \node[coll, above=0.5cm of products] (cat) {categories};
    \node[coll, below=0.5cm of products] (sup) {suppliers};
    \node[coll, below=0.5cm of sup] (rev) {reviews};

    \draw[rel] (addr) -- (users);
    \draw[rel] (carts) -- (users);
    \draw[rel] (orders) -- (users);
    \draw[rel] (items) -- (orders);
    \draw[rel] (pay) -- (orders);
    \draw[rel] (items) -- (products);
    \draw[rel] (products) -- (cat);
    \draw[rel] (products) -- (sup);
    \draw[rel] (rev) -- (products);
    \draw[rel] (rev) to[bend right=10] (users);
  \end{tikzpicture}

  \vspace{0.8em}
  \footnotesize Les flèches représentent les références (\texttt{ObjectId}) entre collections.
\end{frame}
